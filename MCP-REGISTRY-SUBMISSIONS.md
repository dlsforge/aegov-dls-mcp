...
- [x] Smithery listed — DONE 2026-07-25: `alamkhandurrani/aegov-mcp` published as a stdio MCPB bundle, release SUCCESS, all 7 tools with schemas verified via `registry.smithery.ai/servers/alamkhandurrani/aegov-mcp`. Page: https://smithery.ai/servers/alamkhandurrani/aegov-mcp
  ...
  - **Future releases:** `node scripts/build-mcpb.mjs` then `npx smithery@latest auth login` (browser approve) then `npx smithery@latest mcp publish ./mcpb-dist/aegov-mcp-<v>-smithery.mcpb -n alamkhandurrani/aegov-mcp`. The CLI caches its API key in `%APPDATA%/smithery/settings.json`, so `auth login` is usually only needed once per machine — the 0.2.2 refresh (2026-07-30, release SUCCESS, 7 tools verified on the registry card) reused the 2026-07-25 auth without a browser.
