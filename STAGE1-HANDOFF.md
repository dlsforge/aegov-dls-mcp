...
### 10.9 Step 6 DONE: full eval round 10/10 — Stage-1 exit test met (recorded 2026-07-09)

- **10/10 screens pass** `npm run evals` (build clean, `npm run smoke` 19/19). Every screen was generated by fresh, **check-blind** MCP-connected assistant agents (one per spec, Opus 4.8) from the prompt alone, via the `mcp__aegov-dls__*` tools; no output was hand-edited. This satisfies the CLAUDE.md Definition of Done.
- **Two upstream tool improvements drove the round** (never hand-corrections): (1) `getComponent`/`listComponents` now emit a `driftWarning` on docs examples that use classes not shipping in 3.0.7 (e.g. `aegov-pagination-larger/smaller`, `aegov-newslette*`, hero `aegov-slider-next/prev`) and advertise the standard page furniture (header/footer/page-rating) — committed in 41c5dcf. (2) **validate_snippet EID false-positive fixed** (`src/tools/validateSnippet.ts`): the `looksEid` heuristic tested the whole `<input>` tag, so a site-search box with `value="Emirates ID renewal"` was flagged as an Emirates ID field missing its pattern. Fix: strip the `value` attribute before the heuristic — a field's identity is its name/id/placeholder/label, not the user-entered value. Smoke's EID case still fires (it keys on `placeholder="784-..."`).
- **Operational note for future eval rounds:** the round was run under heavy platform stream instability — agents fetched components fine (small calls) but the API repeatedly closed the stream when emitting the full ~80KB page in one message. Two mitigations worked: (a) resume the dead agent from transcript (it keeps its fetched components) and (b) a **chunked heredoc-append write** (`cat >> file <<'EOF'` per section, then Read-back → validate_snippet) so a cutoff loses one small chunk, not the whole file. Screens 01 and 04 needed the chunked path; 02/03/05/07/08/10 landed on retry.
- **Screen-10 (Arabic RTL) high-stakes verification:** `dir="rtl"` set, Emirates ID input carries `pattern="^784-\d{4}-\d{7}-\d$"`, **zero** unmasked full-format EID values in the page (masked read-back uses X's; the pre-filled `data-full` digits + reveal control were removed), and 11 generated-Arabic strings carry `<!-- AR: needs native-speaker review -->` markers. **Alam still owes the native-speaker Arabic pass** across all generated strings (both scaffolders + the RTL screen).
- **Round-2 product findings (still open, for post-publish iteration):** the §10.8 findings stand — `getComponent` header payload ~50k chars (agents spill it to a file); tools give no CSS/JS integration guidance (how a page loads the Tailwind-v4 plugin). Newsletter/hero/pagination docs examples ship drift classes the agents must strip each time — consider a package-clean canonical-markup mode so the driftWarning becomes belt-and-suspenders rather than required cleanup.
- **Next: step 7** — README (must carry the "Community project. Not affiliated with or endorsed by TDRA." line) + npm publish under `@dlsforge`. Gated on Alam's explicit go.
