...
> **Build log (2026-07-12):** step **0 DONE** (d6a4d91 — monorepo + rules-core extracted, all 84 Stage-1 tests preserved + 17 new = 101/101, evals 10/10, smoke 19/19, validate OK, T2 closed). Step **1 DONE** (f087601 — aegov-audit renders u.ae/en, u.ae/ar with dir=rtl, designsystem.gov.ae). Step **2 DONE** (47c63a2 — axe-core 4.12.1, normalized AuditFinding shape, seeded fixture 7/7). Step **3 DONE** (04a2edb — Lighthouse 13.4.0, both form factors, run conditions documented, NO thresholds asserted; note: lighthouse needs node ≥22.19, aegov-audit engines bumped accordingly). Step **4 DONE** — (a) shared rules-core checks over the rendered DOM (caught the live `aegov-newslette` docs typo on designsystem.gov.ae); (b) **token fidelity (T4)**: inline hard-coded design values on aegov-* elements (design properties only — JS-written positioning and zero-px excluded after real-site false positives) + computed colors vs the 173 oklch tokens (browser-converted, exact) as review flags; (c) **structure (T5)**: check-item/modal/accordion invariants read off the docs examples, docs-tier confidence with source URLs; (d) **UAE Pass**: login surface (password input / login control) with no UAE Pass signal → serious heuristic flag; (e) **Arabic/RTL parity** via `--parity [url]` (explicit or hreflang-discovered): dir=rtl on the Arabic variant, lang tagging, count-drift beyond tolerance, aegov components missing in one variant — every parity finding is a human-review flag, never an assertion. 25/25 audit tests across 4 seeded fixtures; real parity run u.ae/en↔/ar flagged both variants missing `<html lang>`. TDRA criteria **VERIFIED** (v2.0 confirmed latest 2026-07-12; 125 items extracted verbatim to `packages/aegov-audit/reference/tdra-assessment-criteria.json`; thresholds live on the criteria page, not the workbook — see §8). Step **5 DONE** — document-meta engine (doctype/charset/viewport/canonical/lang/dir/alternate → items 3.26–3.35), LCP/FCP metrics added to Lighthouse (verified targets ≤2.5 s/≤1.8 s), and the report layer: machine `report.json` + human `report.md` mirroring the official checklist — 14 of 125 items machine-checked (3.2, 3.4, 3.6, 3.10, 3.12, 3.24, 3.26–3.28, 3.32–3.35, 3.48; curated mapping in `src/report/tdra.ts`, pinned by test), thresholds evaluated ONLY under the local-run caveat, run conditions + five disclaimers in every report, `--out <dir>` writes both files. 31/31 audit tests. Recorded real report run against designsystem.gov.ae. Step **6 DONE** — the CI surface: (a) `--fail-on <critical|serious|moderate|minor|none>` on the CLI (default `none` — report-only behavior unchanged; exit 1 when findings sit at or above the threshold, exit 2 on usage errors); (b) the reusable composite action at `packages/aegov-audit/action/` (secondary decision settled: same package/repo, consumed as `dlsforge/aegov-dls-mcp/packages/aegov-audit/action@<ref>` — no separate action repo) — installs a pinned `@dlsforge/aegov-audit` (or `install-spec` tarballs until step 8 publishes), runs the audit, `annotate.mjs` turns report.json into ::error annotations at/above `fail-on` and ::warning for the rest, report.md → job summary, report artifact, final gate step fails the build (inputs reach scripts only via `env:` — injection-safe, pinned by test); (c) repo CI: `.github/workflows/ci.yml` (the full monorepo gate on every push/PR) and `mizan-selftest.yml` (dogfoods the action end-to-end on GitHub runners via the §11 two-tarball flow: fail-on none must pass and count the seeded defects, fail-on critical must fail the build). 45/45 audit tests (146 total across the monorepo); the whole install→run→annotate path also dry-run verified locally from packed tarballs (2 criticals → exit 1, counts in step outputs). **Both workflows verified green on real GitHub runners (98f000a)** after one Linux-only fix: ubuntu-24.04 runners restrict unprivileged user namespaces, killing raw Chromium's sandbox under chrome-launcher (ECONNREFUSED) — Lighthouse now adds `--no-sandbox` on linux+CI only, records chromeFlags in every report's runConditions, and honors a CHROME_PATH override. Step **7 DONE (the exit test)** — (a) `evals/` in aegov-audit: 8 fixtures (1 fully compliant bilingual pair pinned to ZERO findings incl. the parity pass; 7 seeded-defect sites with expected + forbidden rule lists), `run-evals.mjs` drives the real CLI, `npm run evals:audit` at root, wired into CI; (b) recorded real run against **fnrc.gov.ae** (owner's site, full report committed with owner approval at `evals/recorded/fnrc.gov.ae-2026-07-12/` — report.json + report.md + NOTES.md); (c) cross-checked against **three independent blind expert reviews** (WCAG, UAE-standards, technical-SEO) of the rendered DOM — verdict: no fabricated failure, no critical machine-checkable miss; (d) the comparison drove real engine iteration, never report edits: navigation-settle handling (`engines/settle.ts` — fnrc chains client redirects), ancestor-aware `dls-arabic-rtl` on the rendered DOM (the core's string rule stays snippet-scoped; language-switcher links no longer false-positive), NEW rules `meta-lang-mismatch` (fnrc serves Arabic under lang="en", 82% Arabic script), `dls-img-alt-placeholder` (alt="Alternate Text"/"Image Placeholder" shipped live), `dls-not-used` (zero aegov-* classes — fnrc is Bootstrap, not DLS), per-rule sampling + rollup (79 raw img-alt hits → 10 + "…and 67 more"), and `finalUrl` recorded after redirects; (e) manual-only boundaries documented explicitly in NOTES.md (keyboard operability, 2.2.2 carousels, hidden-content, language-of-parts, content quality, Arabic wording). fnrc parity leg documented as inapplicable: both languages share one URL via a culture cookie — flagged via meta-alternate, manual click-through confirmed structural equivalence. Gate: 146/146 tests, mcp evals 10/10, audit evals 8/8, smoke 19/19, validate OK. Step **8 DONE (2026-07-13) — STAGE 2 COMPLETE.** READMEs for all three packages (rules-core had none), LICENSE added to aegov-audit (dry-run caught its tarball shipping without one), `PUBLISHING.md` runbook. **Published by Alam: `@dlsforge/aegov-rules-core@0.1.0` then `@dlsforge/aegov-audit@0.0.1`** (mcp@0.1.1 stays deferred per §11) — verified live with a clean registry install into an empty dir (rules-core resolved as a real dependency, bin runs, `--fail-on critical` exits 1 on the seeded fixture). Post-publish cleanups: G2 restored to the single-tarball flow (core resolves from the registry — the real `npx` path), action README's tarball caveat replaced (default `version` install now resolves; `install-spec` kept for this repo's self-test of current source), releases tagged `aegov-rules-core-v0.1.0` + `aegov-audit-v0.0.1`. One environment fix en route: the publish session brought npm 12, whose `pack --json` emits an object keyed by package name instead of an array — `packDryRunFiles()` in the test helper now accepts both shapes. Remaining carry-overs (not Stage-2 blockers): Arabic native-speaker review, MCP-registry listings, the deferred mcp@0.1.1 republish. (evals fixtures + recorded real run; README + publish).
