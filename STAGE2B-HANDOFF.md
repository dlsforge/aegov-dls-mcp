...
> **DEFERRED (2026-07-21) → CLOSED (2026-07-26):** 3.19–3.22 and 2.40 were held back for want of a trustworthy header/footer contract, and no weaker heuristic was substituted. They now ship, taking coverage **61 → 66/125**.
>
> The 2026-07-21 rationale was imprecise about what was missing. The docs-sourced block *markup* had been in `catalog.blocks` since Stage 1 (8 blocks, with markup, examples and `packageClassRoots`). What did not exist was a checkable *contract*: the docs header example is ~50 KB of one entity's menu content, so "does the page match the example" is not a question any real site passes. rules-core now carries curated `blockContracts` — the small set of invariants the docs actually mandate, each citing the sentence or markup that mandates it (verified verbatim at build time) and pinned to the source page's `contentHash` so docs drift surfaces instead of silently invalidating a rule. See `scripts/block-contracts.mjs` and `src/rules/blocks.ts`.
