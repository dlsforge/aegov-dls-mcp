...
- **Origin probes** (http(s) targets) — `Sitemap.xml` (robots.txt-aware; checklist 3.64, the crawler artefact), a **human-facing site-map page** (checklist 1.5, found via the page's own labelled link or well-known paths — these are two different requirements and neither satisfies the other), and designed 404 error pages (soft-404 and bare-server-default detection).
...
- **Interaction checks** — layout reflow at 175% zoom and at every design-system breakpoint (horizontal-overflow detection), a bounded real-keyboard tab walk (regions whose controls are never reachable), and visible focus indication on every element the walk visits (fail-soft: an aborted walk reports "not checked", never a guess).
- **Offline W3C-style validation** — the raw page source (not the browser-repaired DOM) through a pinned `html-validate` with spec-conformance rules only; no calls to validator.w3.org.
- **Technology fingerprints** (hard, self-declared signals only — no signal, no finding) — a monolithic CMS rendering the page (generator meta / core asset paths; the checklist asks for headless + JS-framework), and icon libraries other than the guideline's approved Phosphor set (docs-sourced, marked for confirmation).
- **Bounded same-origin crawl** (http(s) targets; home + up to 6 linked pages, robots.txt honoured, self-identifying User-Agent; `--no-crawl` to skip) — unique per-page titles/descriptions, evidence that the CMS manages meta tags **at page level** (checklist 3.62: titles and descriptions that actually vary per page — reported as "no evidence", never as "no ability"), alternate-hreflang on every crawled page, and a heuristic Page-Rating-block presence check on service-looking pages.
- **Header/footer block conformance** — the page's header and footer against the design system's documented blocks: the block roots, the mobile menu and footer accordion, the documented 7-item limit on a primary navigation, and a copyright year that actually changes. The invariants come from `rules-core`'s docs-sourced block contracts, each citing the docs sentence or markup that mandates it; a requirement whose anchor is absent reports **"not checked"** rather than passing something it never saw.
