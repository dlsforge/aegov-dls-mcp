...
> @dlsforge/aegov-audit@0.0.1 · 2026-07-19T22:05:07.369Z · win32 x64, node v24.16.0
> Page: HTTP 200, 1395 nodes in 244 ms — "مؤسسة الفجيرة للموارد الطبيعية" (lang=en dir=(unset))
> Final URL after redirects: https://fnrc.gov.ae/portal/home/index

**58 finding(s)** — 1 critical, 19 serious, 33 moderate, 5 minor · engines: axe 4.12.1, dls @aegov/design-system@3.0.7
...
| largestContentfulPaint | mobile | 28846 | <=2.5s (2500 ms) | **no** |
| firstContentfulPaint | mobile | 5677 | <=1.8s (1800 ms) | **no** |
| accessibility | desktop | 73 | >=90 | **no** |
| performance | desktop | 24 | >=90 | **no** |
| seo | desktop | 82 | >=90 | **no** |
| bestPractices | desktop | 69 | >=80 (preferably >=90) | **no** |
| largestContentfulPaint | desktop | 4755 | <=2.5s (2500 ms) | **no** |
| firstContentfulPaint | desktop | 2026 | <=1.8s (1800 ms) | **no** |
...
Mizan machine-checks 39 of 125 checklist items (fully or partially). "No automated findings" covers the machine-checkable subset only — it is NOT a pass; the remaining items are process/design questions a human answers. Items marked "not checked" had no evidence engine in this run (they need --lighthouse and/or an http(s) target for the origin probes).
...
- **✗ 3.53** Eliminating all images on the page, is your page weight below 500KB while loading?
  - 1 finding(s)
  - [serious|external] lh-page-weight-no-images: Page weight excluding images exceeds the TDRA budget of 500 KB (checklist 3.53): mobile 1.51 MB, desktop 2.23 MB. Transfer size of this local load, images subtracted. → fix: Trim non-image payload: split/defer JavaScript, remove unused CSS, subset fonts, enable text compression.
- **✗ 3.54** With all images on your page, is your page weight below 4MB while loading?
  - 1 finding(s)
  - [serious|external] lh-page-weight-total: Total page weight exceeds the TDRA budget of 4 MB (checklist 3.54, read as 4.00 MB): mobile 8.51 MB, desktop 9.24 MB. Transfer size of this local load. → fix: Reduce transferred bytes: compress/resize images, remove unused scripts and styles, enable text compression.
...
_86 of 125 checklist items need human review (process/design questions outside automated scope)._
