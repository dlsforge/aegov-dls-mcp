...
> @dlsforge/aegov-audit@0.1.0 · 2026-07-26T18:12:52.356Z · win32 x64, node v24.16.0
> Page: HTTP 200, 1395 nodes in 430 ms — "مؤسسة الفجيرة للموارد الطبيعية" (lang=en dir=(unset))
> Final URL after redirects: https://fnrc.gov.ae/portal/home/index

**89 finding(s)** — 1 critical, 23 serious, 57 moderate, 8 minor · engines: axe 4.12.1, dls @aegov/design-system@3.0.7
...
| seo | mobile | 82 | >=90 | **no** |
| bestPractices | mobile | 85 | >=80 (preferably >=90) | yes |
| largestContentfulPaint | mobile | 23529 | <=2.5s (2500 ms) | **no** |
| firstContentfulPaint | mobile | 4200 | <=1.8s (1800 ms) | **no** |
| accessibility | desktop | 73 | >=90 | **no** |
| performance | desktop | 43 | >=90 | **no** |
| seo | desktop | 82 | >=90 | **no** |
| bestPractices | desktop | 85 | >=80 (preferably >=90) | yes |
| largestContentfulPaint | desktop | 3060 | <=2.5s (2500 ms) | **no** |
| firstContentfulPaint | desktop | 2092 | <=1.8s (1800 ms) | **no** |
...
Mizan machine-checks 66 of 125 checklist items (fully or partially). "No automated findings" covers the machine-checkable subset only — it is NOT a pass; the remaining items are process/design questions a human answers. Items marked "not checked" had no evidence engine in this run: they need --lighthouse, an http(s) target (origin probes and the bounded crawl), --entity-type ministry (item 2.12), a keyboard walk that completed without aborting, or — for the header/footer block items — the block itself present on the page to be judged against its documented contract.

### SECTION 1 : STRATEGY, USER EXPERIENCE AND CONTENT

- **✗ 1.1** Have you implemented a version of the design system greater than 1.0?
  - 1 finding(s)
  - [serious|heuristic] dls-not-used: No AEGOV Design Language System components detected on this page (zero aegov-* classes) — the UI appears hand-rolled or built on a third-party framework. The standard requires official DLS components  → fix: Adopt @aegov/design-system components and tokens for the page's UI.
### SECTION 2 : DESIGN
...
- **✗ 2.7** Has the design team adhered to the primary text colour of AEBLACK-800?
  - 1 finding(s)
  - [moderate|heuristic] style-text-primary: The dominant body-text colour computes to rgb(158, 151, 143) (no DLS token match) across 60% of sampled text — the checklist asks for AEBLACK-800 as the primary text colour. → fix: Set primary body text to the aeblack-800 token.
...
- **✗ 2.21** Are you using the approved library for use of icons?
  - 1 finding(s)
  - [minor|docs] stack-icon-library: Font Awesome is in use on this page (stylesheet https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css) — the iconography guideline names Phosphor as the design system's approved  → fix: Migrate icons to the approved Phosphor library, or record the exception for the reviewer.
...
- **• 2.38** Has the design team tested the layout when zoomed in to a browser at 175%?
  - no automated findings (subset only — not a pass)
- **✗ 2.40** Is the design for the website using the approved header and footer elements without change?
  - 2 finding(s)
  - [serious|docs] blk-header-root: The page header must be the DLS header block — a <header> element carrying the aegov-header class. → fix: Use the DLS header block: <header class="aegov-header"> … </header> (designsystem.gov.ae/docs/blocks/header).
  - [serious|docs] blk-footer-root: The page footer must be the DLS footer block — a <footer> element carrying the aegov-footer class. → fix: Use the DLS footer block: <footer class="aegov-footer"> … </footer> (designsystem.gov.ae/docs/blocks/footer).
...
- **✗ 3.1** Have you installed the latest version of the design system via NPM?
  - 1 finding(s)
  - [serious|heuristic] dls-not-used: No AEGOV Design Language System components detected on this page (zero aegov-* classes) — the UI appears hand-rolled or built on a third-party framework. The standard requires official DLS components  → fix: Adopt @aegov/design-system components and tokens for the page's UI.
...
- **✗ 3.12** Have you tested your frontend code to be AA Level compliant with WCAG 2.1
  - 5 finding(s)
  - [serious|external] aria-hidden-focus: ARIA hidden element must not be focusable or contain focusable elements → fix: Fix all of the following:
...
- **• 3.15** Have you carried out Mobile responsiveness testing?
  - no automated findings (subset only — not a pass)
- **✗ 3.19** Are you using the approved header layout and design as mentioned on designsystem.gov.ae/docs/blocks/header
  - 1 finding(s)
  - [serious|docs] blk-header-root: The page header must be the DLS header block — a <header> element carrying the aegov-header class. → fix: Use the DLS header block: <header class="aegov-header"> … </header> (designsystem.gov.ae/docs/blocks/header).
- **○ 3.20** Have you used the header code as described for mobile use?
  - not checked in this run — its evidence engine did not run (see the checklist note)
- **✗ 3.21** Are you using the approved footer layout and design as mentioned on designsystem.gov.ae/docs/blocks/footer
  - 1 finding(s)
  - [serious|docs] blk-footer-root: The page footer must be the DLS footer block — a <footer> element carrying the aegov-footer class. → fix: Use the DLS footer block: <footer class="aegov-footer"> … </footer> (designsystem.gov.ae/docs/blocks/footer).
- **○ 3.22** Have you used the footer code as described for mobile use?
  - not checked in this run — its evidence engine did not run (see the checklist note)
...
- **✗ 3.40** Are all pages and all HTML code W3C compliant?
  - 1 finding(s)
  - [moderate|external] w3c-invalid-html: html-validate 11.5.6 (offline, preset "standard" — HTML5 spec conformance only) reports 98 validation error(s) in the page source across 3 rule(s) (element-permitted-content×51, element-permitted-pare → fix: Fix the listed markup errors at the source/template level; re-run to confirm zero errors.
- **• 3.41** Have you ensured the use of Google Fonts and not loading fonts from your server?
  - no automated findings (subset only — not a pass)
- **• 3.42** Have you tested all pages on responsive breakpoints mentioned in the design system?
  - no automated findings (subset only — not a pass)
...
- **✗ 3.53** Eliminating all images on the page, is your page weight below 500KB while loading?
  - 1 finding(s)
  - [serious|external] lh-page-weight-no-images: Page weight excluding images exceeds the TDRA budget of 500 KB (checklist 3.53): mobile 2.17 MB, desktop 2.15 MB. Transfer size of this local load, images subtracted. → fix: Trim non-image payload: split/defer JavaScript, remove unused CSS, subset fonts, enable text compression.
- **✗ 3.54** With all images on your page, is your page weight below 4MB while loading?
  - 1 finding(s)
  - [serious|external] lh-page-weight-total: Total page weight exceeds the TDRA budget of 4 MB (checklist 3.54, read as 4.00 MB): mobile 9.49 MB, desktop 9.47 MB. Transfer size of this local load. → fix: Reduce transferred bytes: compress/resize images, remove unused scripts and styles, enable text compression.
...
- **• 3.59** Have you added a cookie banner?
  - no automated findings (subset only — not a pass)
- **• 3.60** Are you using a Headless CMS?
  - no automated findings (subset only — not a pass)
- **• 3.61** Does your technology strategy involve the use of a JavaScript Framework (Such as Next.js) with a headless CMS for content management?
  - no automated findings (subset only — not a pass)
...
_59 of 125 checklist items need human review (process/design questions outside automated scope)._
